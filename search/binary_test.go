...
func TestRecursiveBinary(t *testing.T) {
	s1 := []int{1, 2, 4, 5, 8, 10, 11, 25, 44, 57, 68, 79, 82, 99, 102, 105}
	t.Run("Slice one should return index", func(t *testing.T) {
		needle := 5
		want := 3
		found, index := RecursiveBinary(s1, needle)
		if !found {
			t.Errorf("RecursiveBinary(s1, %v) did not find item when it exists in the slice", needle)
		}
		if index != want {
			t.Errorf("RecursiveBinary(s1, %v) expected index %v, got %v", needle, want, index)
		}
	})
	t.Run("Slice one should not find needle", func(t *testing.T) {
		needle := 45
		found, index := RecursiveBinary(s1, needle)
		if found {
			t.Errorf("RecursiveBinary(s1, %v) found item when it does not exist in the slice", needle)
		}
		if index != -1 {
			t.Errorf("RecursiveBinary(s1, %v) expected index -1, got %v", needle, index)
		}
	})
	...
	t.Run("Slice two should return index", func(t *testing.T) {
		needle := 2
		want := 4
		found, index := RecursiveBinary(s2, needle)
		if !found {
			t.Errorf("RecursiveBinary(s2, %v) did not find item when it exists in the slice", needle)
		}
		if index != want {
			t.Errorf("RecursiveBinary(s2, %v) expected index %v, got %v", needle, want, index)
		}
	})
	t.Run("Slice two should not find needle", func(t *testing.T) {
		needle := 45
		found, index := RecursiveBinary(s2, needle)
		if found {
			t.Errorf("RecursiveBinary(s2, %v) found item when it does not exist in the slice", needle)
		}
		if index != -1 {
			t.Errorf("RecursiveBinary(s2, %v) expected index -1, got %v", needle, index)
		}
	})
	...
	t.Run("Slice three should return index", func(t *testing.T) {
		needle := 16
		want := 3
		found, index := RecursiveBinary(s3, needle)
		if !found {
			t.Errorf("RecursiveBinary(s3, %v) did not find item when it exists in the slice", needle)
		}
		if index != want {
			t.Errorf("RecursiveBinary(s3, %v) expected index %v, got %v", needle, want, index)
		}
	})
	t.Run("Slice three should not find needle", func(t *testing.T) {
		needle := 9999
		found, index := RecursiveBinary(s3, needle)
		if found {
			t.Errorf("RecursiveBinary(s3, %v) found item when it does not exist in the slice", needle)
		}
		if index != -1 {
			t.Errorf("RecursiveBinary(s3, %v) expected index -1, got %v", needle, index)
		}
	})
}
